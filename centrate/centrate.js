...
Control.prototype.on_ask = function (q) {
    ...
    if (x===4){
        this.gui.append_ask_line(MSG['guessed'], MSG['you win']);
        this.gui.game_end();
        return;
    }
...
Control.prototype.on_answer = function (answ) {
    ...
    if (state == 'lie') {
        this.gui.append_answer_line(MSG['you lied'], MSG['you loose']);
        this.gui.game_end();
    } else if (state == 'success') {
        var solver_q = this.solver.solutions[0];
        this.gui.append_answer_line(solver_q, '');
        this.gui.append_answer_line(MSG['guessed'], MSG['I win']);
        this.gui.game_end();
    } else {
        this.gui.focus_player();
    }
